...
#+LAST-MODIFIED: [2025-04-10 Thu 14:45]
...
- [[#org-mode][ORG MODE]]
  ...
  - [[#org-agenda-done-tasks][Org-Agenda-Done-Tasks]]
  ...
  - [[#org-highlight-todo][Org Highlight TODO]]
  - [[#org-roam][Org-roam]]
...
I have also attempted to account for HiDPI displays by doubling the font size if the screen size is larger than 1080 pixels in height. (Edit: This is now commented out as I've switched to Wayland native Emacs on Linux which fully supports HiDPI)
...
Kele is a Kubernetes manager, allowing for easy managment of a Kubernetes cluster directly from Emacs. As it's an external package, Doom Emacs has no keybindings for it, so we're going to define some here.

| COMMAND             | DESCRIPTION    | KEYBINDING |
|---------------------+----------------+------------|
| kele-config         | Config         | SPC k .    |
| kele-context-switch | Switch context | SPC k c s  |
| kele-get            | Get            | SPC k g    |
| kele-mode           | Kele mode      | SPC k k    |
| kele-list           | List           | SPC k l    |
...
* NO TITLE BAR
To get a little more real estate, we can remove the title bar, currently only set up on macOS:
...
| ~/Org/agendas/business.org | Edit business agenda file   | SPC = a b  |
...
This is config for Org Mode, it has been mainly wrapped in `(after! org)` as without it, it may load in before Doom's config and get overritten later. I have also enabled org-journal, org-modern, and org-roam by adding (+journal +pretty +roam2) to the org section of my Doom Emacs init.el.
...
** Org-Agenda-Done-Tasks

I'd like an easy way to archive completed tasks. So I've written a custom command =rh/org-archive-done-tasks= and set up a keybinding to run it easily:

| COMMAND                   | DESCRIPTION                            | KEYBINDING |
|---------------------------+----------------------------------------+------------|
| rh/org-archive-done-tasks | Archives all completed tasks in agenda | SPC m D    |
...
Org-tempo allows for quick insertion of templates. Great for code blocks. I have added a few custom template shortcuts. For example, creating the block below, in emacs-lisp can be achived by =C-c C-, el=.
...
By default, inline images are toggled with =z i=, however, I'm probably going to forget this, and would rathert have it tucked away in the Toggles section, so we're going to map it to =SPC t i=:
...
* VBA MODE
Some config to enable vba-mode from Yoshihiko Kakutani on GitHub: https://github.com/ayanyan/vba-mode
